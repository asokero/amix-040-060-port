...
4. If 060SP does **not** do full emulation, say so plainly and state what a from-scratch
   ...
   restore. Confirm or correct that list — and note that `prototypes/lmul060.s` shows the
...
3. Does `fpu_present` get set correctly for an FPU-less part? It derives from AmigaOS
   `AttnFlags` via the loader (`cputype` is poked the same way, see `prototypes/cputype060.s`).
...
`prototypes/lmul060.s`'s header records a 2026-07-09 census concluding that `lmul`'s three
...
- Implementation of anything here waits until after the hardware session
  (`REALHW-VERIFY-260725.md`), which validates a large 040 delta on real silicon. The 060 has no
