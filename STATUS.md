...
| Denied write-back propagation | ? | **OPEN** | — | — | **ISSUE-42 — release blocker** |
...
| 42 | denied write-back replay is swallowed → silent lost store | **OPEN — BLOCKER** | 040 | contract solved (Codex `8fd31fd`), implementation not written; needs the A3640 swap |
...
1. **ISSUE-42** — a denied write-back replay is swallowed on the 68040, which can leave a silent
   partial store. This is the only known correctness defect that can corrupt user data without
   any visible symptom. Contract is solved; the implementation is not written, and it needs the
   A3640 card swap to verify. **Blocks calling the 040 port "done".**
