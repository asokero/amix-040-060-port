...
| 54 | a WD **phase mismatch** on any target kills the A3091 driver permanently | **OPEN** | 060 HW, **reproducible on demand** | `itab[0x49]` = illegal/unsupported, `atab[STARTING][1]` = DEAD. `0x42` (absent target) is handled gracefully; `0x49` (`SBIC_CSR_MIS_1|phase`) is not in the table at all. Reading `/dev/rdsk/c3d0s0` takes the whole machine down. Same family as 53, different instance; **not evidence about 53**. Provoked on demand 2026-08-27 with the entry-ISTR print live: `entry=D0` = `INT_F|INTS|INT_P`, a genuine WD interrupt, so the DFA owns it entirely and the demux wrapper is exonerated. 8/8 predictions |
| 55 | the cross compiler existed only as an uncommitted working-tree change | **OPEN** upstream | build host | `BUILDING.md` told a reader to build the toolchain from a repository that produced a **different compiler** from the one behind every hardware acceptance here. 66 lines of SGS repairs, committed and offered upstream; reconciliation measured byte-neutral on 020/030/040. Closes when the PR lands and this clone updates |
