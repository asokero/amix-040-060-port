...
|
| Port spec: prototypes/hat-040-port-worklist.md "hat_dup -- PORT PLAN (2026-07-04)".
...
|   * hat_ptalloc flags arg: Part B leaf alloc keeps the original's 2 (bit1 =
|     do NOT steal an active table, bit0 clear = do NOT sleep -> may return 0,
|     handled like the original: give up, return 0).  Part A uses 1 (sleep until
|     memory, never steal) = the proven hat040.s lazy-root pattern.
...
| page-aligned => 512-aligned; flags=1: sleep for memory, never steal)
	pea	1
	movel	%fp,%d0
	subil	&76,%d0
