...
`src/mk_modelb_sysroot.sh`, `src/modelb_geom_probe.c`,
...
`src/mk_modelb_sysroot.sh` builds `build/sysroot-modelb` as a symlink mirror of the real
...
`src/check_page_geometry.sh` asserts in the **compiled bytes** that no object shifts by 11.
