...
Read this and `docs/archive/RESUME-HERE-260731.md` (the standing record of where the port stands). Everything
...
and runs X11R5 with `twm` and `xterm` on screen. Full record: `docs/REALHW-ACCEPTANCE-260801.md`.
...
|---|---|---|
| `codepub040` | `mprotect(..., PROT_EXEC)` is the user-code cache-publication barrier | `docs/USER-CODE-PUBLISH-260801.md` |
| `include-modelb/` + mirror sysroot | 4 KiB page geometry for everything compiled into this kernel | `docs/MODELB-HEADERS-260801.md` |
| `issue39_040` + `memwatch.c` + `leaktest.c` | the kernel's COMMON memory globals are readable at last | `docs/ISSUE39-MEMORY-REGIME-260801.md` |
...
* **A shift check that only matched `lsrl` was blind to signed `>> 11`**, which compiles to
  `asrl` — exactly what `va2000.c` emits. `src/check_page_geometry.sh` matches both.
...
> **★ All five pre-registered criteria met: `docs/REALHW-ISSUE40-ACCEPTANCE-260802.md`.** Clean boot of
...
> `keep0 = keepN = meta = badlink = wake = 0`. Two units: `src/legacysdt040.s` +
> `src/ptdatfree040.s` with `hat_ptfree` V3. Open and logged, not chased: the `sh: no space`
> harness failure, and `ptd_wake_n` still unexercised (the `pt_waiting` path has never fired).

> **★ Both halves landed and hardware-proven: `docs/REALHW-ISSUE40-CLOSED-260802.md`.** Kernel
...
> Codex's legacy-SDT teardown edge is implemented (`src/legacysdt040.s`, called from
...
> `docs/ISSUE40-LEGACY-SDT-LANDED-260801.md`; contract request: `docs/archive/ISSUE40-PTDAT-CODEX-QUESTIONS.md`.
> **RAUTAVAHVISTUS samana iltana** (`docs/REALHW-ISSUE40-PART1-260801.md`, NAS `amix/hwtest-260801b/`):
...
Start here: `docs/archive/NEXT-SESSION-ISSUE40-PROMPT.md`. Open questions for Codex, if wanted before
implementation: `docs/archive/ISSUE40-CODEX-FOLLOWUP-QUESTIONS.md`.
...
* **`Xrtg` is not on this root disk or the NAS**, so X on the VA2000 was not exercised *this
  ...
  with several clients — `docs/REALHW-ACCEPTANCE-260731.md`). The only open part is re-verification on
...
Unchanged from `docs/archive/RESUME-HERE-260731.md` §7: measurement boot on the existing dual-CPU kernel →
