...
The four-site unit landed as one fail-closed patch (`src/patch_nfs_getpage.py`, 9 canaries,
...
the 060. `src/wb040.s`'s `wb060_xpage` cites Linux/m68k's `if (fslw & MA) addr = (addr+7) & -8`,
...
reaches it. Fix (`src/krnxmemflt040.s`): after a **successful** resolve, if the fault address
