...
> Commit `8ce15ee`. Evidenssit `test-tools/issue34-060-unimpl-integer-260727.txt` ja
...
> Commitit `32608ca` (FPSP→base) ja `e2f79e2` (Xsvga-polku + yhdistetty RTG-kerneli).
...
> Nyt `src/xsvga-provenance.sh`: polku + `sha256 97a9a39d…c72301` (60464 B),
...
>   (build 68040-260727-04, **1 716 847 B**, `serdbg_putc` linkattu ja `conputc` uudelleen
>   osoitettu), joten build-on-demand ei ole oletus vaan
...
>   Pinni verifioitu tavuntarkasti (HEAD `9d7ffe9`, `unix-040` `767ea9a0…`, `.text` `9b49c77a…`).
...
>
> **RAUTASESSIO on yhä suurin avoin kohde**, ajolista `docs/REALHW-VERIFY-260725.md`
...
> **FIX:** `src/prfastmap040.s` = `uvatopte040` (040 per-proc root→ptr→leaf,
...
> raudalla. **Ajolista on omana tiedostonaan: `docs/REALHW-VERIFY-260725.md`** (artefaktit +
> tarkat komennot + odotettu tuloste per kohta + hyväksymistaulukko + burst4.sh);
> taustat ja koneen fysiikka: `docs/archive/RESUME-HERE-040-HARDWARE.md` ylälohko.
...
> **2. FPSP M1–M4 (e1a2615 + c6e8378 + 0b71252 + 3cf4fb2).** M1 `build-fpsp040.sh` (Motorolan
...
> **4. EI-DEBUG-VARIANTIT (f5d59ba).** `relink-040-fpsp-xsvga.sh` sai `$2`=OUT-parametrin →
> `unix-040-fpsp-xsvga` (**260724-11**, base `unix-040`) rinnalle `-dbg`:n (**260724-09**).
> Emu-verifioitu: bootaa puhtaasti, FPUTEST Test A PASS.
> **5. VA2000-KERNELI RAUTATESTIIN (f4884e5).** MNT VA2000 RTG -ajuri (`~/kehitys/va2000-amix`
...
> ## 🔶 2026-07-23 (myöhäisilta) — B2-CB-HOOKS LANDATTU + EMU-VERIFIOITU (381829b); RAUTA-AJO SEURAAVANA PÄIVÄNÄ
...
> ## ✅✅✅ 2026-07-23 — B1-DC-ENABLE LANDATTU + RAUTAHYVÄKSYTTY (bc27d81, tagit pre-dc-enable/b1-dc-enable): WRITETHROUGH-DATACACHE PÄÄLLÄ
> **DC-tien lähtöcommit** — revert = `git tag pre-dc-enable` (c4ef944) tai minimirevert
...
> **FIX (self-contained, loader-riippumaton, commit 8d913b8+f2bf802):** `src/config040.s`
...
> CM:n → DC ei näy edes WT:ssä): Codex-toimeksianto kirjoitettu → **`docs/archive/DTT0-NARROWING-TASK.md`**
...
> Toteutus (`src/dma_cache040.s` + `patch_a3091_dma.py`): jaettu
...
> Dokumentti: docs/CACHES-ON-PLAYBOOK.md "Step B1-DMA". Evidenssi test-tools/cmb1dma-emu-verify-260720.txt.
...
> **(1) swapadd-geometria** (50abe53, `patch_swapgeom.py` 12 sitea / 5 fn) — slottimäärä
...
> (5637b04, `patch_execstk.py`) — .data-init symtab-resolvoitu 0x800→0x1000 + jaettu shift
> 11→12 (yksi moveq syöttää btoc/ctob/64-sivuryhmän); hyväksyntä `test-tools/bigargv.c`:
> 4500 B argv execin yli tavuntarkasti ×3. **(3) pageout-oletukset** (3425d82,
...
> täsmälleen ennustettu vaikutus. **(4) mincore** (c79730a, `patch_mincore.py`) — btoc-vektori
...
> M-harvest (1e1006c) + klustsize 0x800 -data-init & swap-in-setti + hat_pteload::Lreplace
> U/M-harvest (9c2ab66, `src/patch_swapin.py`). Buildit 260719-01 (base) / -02 (dbg)
> / -03 (quiet). EMU: burst4 ALLBURSTS-DONE, 0 uutta busia, EI 4AFC005F:ää. REAL-HW
> (A3000+Mercury040, 32b3296): dbg pressure+burst4 swap elävänä, virtakatkaisu-disk-truth
...
> commit 86bcc91. **Verified emu-040 AND emu-060:** boot→login clean, interactive login
...
> caches Step B; ISSUE-10 free-time reverse-map invariant probe (see KNOWN-ISSUES, e321d5a).
...
> **`src/patch_writeback.py`** — all 20 sites from Codex's
...
> bisection + exercises, full detail in `docs/archive/WRITEBACK-TASK.md`:
...
> **NEXT: retire the `schedpaging` rts-override in `src/runtime040.s`** (its own
...
> `docs/archive/WRITEBACK-TASK.md` (repo root). Order: spec_putpage (4 sites, simplest) → pvn_range_dirty
...
> **Committed `b5521d5` + `62b3c8d`.** The loader (`build/unix_boot040`, rebuild via
...
> **Committed `df7f879`: `src/krnxmemflt040.s`** — the coupled 4-defect stock
...
> **Committed `a84d689`: runtime040.s** — the quiet/dbg overlays' load-bearing overrides
...
> **Committed `46b159c` (on master): `src/bp_map040.s` + relink-040.sh wiring + KNOWN-ISSUES.**
...
> ## ✅ MILESTONE 2026-07-10 — DUAL-CPU KERNEL: 68060 SUPPORT MERGED (branch `060-prestudy` → master, merge `f426290`)
...
> - **wb060_sswsynth** (`af993d3`): fmt-4 FSLW marks locked RMW (TAS/CAS) as read+write;
...
> - **wb060_xpage** (`c4baa3d`): the 060 restart model reports FA = the access's START on
...
> Model-B conversion (c4873ae, real file-tail bugs), ISSUE-11 registered (wb040 WB1
...
>   `ptptr=ctob(nextfree)` / `ksegmappt=ctob` / `nextfree=btoc`), commit **ef3eb90**. Emulators
...
>   rebuild it from the live kptr040 tree, commit **0cc9fb4**. (A symptom of the same halved-leaf
...
>   `rol.l #8` + `moves.b (a3)+`, one independent per-byte access), commit **c0a3cd8** (plus a 68k
>   CC-clobber fix in the first cut). Diagnostic probes that cracked it: commit **3aeeb7c**
...
>
> 3. **BONUS same evening: quiet root-mount ENXIO regression RESOLVED (commit `da9dd06`,
...
- **hat_map phantom-preload disable (commit 178364a) — boot-tested, no regression.** User ran
  ...
  why-Option-A: `src/hat-map-040-fix-plan.md`. ISSUE-7 unchanged (this was a correctness
...
PREEMPT6 (commit d618652) fired: `scanned=16 zerocount=1` — the corruption is confirmed
...
Fixed in `patch_modelb.py` (commit `05d7c99`), all three kernels rebuilt clean — **since
...
- **hat_dup040 merged (ISSUE-4 CLOSED pending boot test):** the real fork/COW port (already
  ...
  now in `src/hat_dup040.s` on master, wired into `relink-040.sh` on top of every newer
...
- **NEW probe added (commit d618652, awaiting boot test): PREEMPT6 in `preempt_dbg.s`** —
...
- **ISSUE-5 (reboot) FIXED:** `reboot` ran an illegal 030 `pmove` (haltsys, reached via `rtnfirm`
  ...
  binary; the AttnFlags guard misfired under the live Unix MMU).  v1/v2/v3 = commits 3d995c1/
  8dfc1d2/8c03c7c.
- **ISSUE-6 (fsck) FIXED:** `fsck` on a dirty UFS panicked `segvn_softunlock`; two Model-B misses —
  ...
  hash sites stayed `>>11` (a hash needs only consistency).  Commits 8127111 + 15e1251.
- **Also fixed:** `hat_unload040` V2.3 (its table-frame guard rejected the kernel-image static
  ...
  commit 1782aaf); `segu_get` SEGU_LOCKED (the Model-B loop-bound patch `moveq #3→#1` also dropped
  the flag stored from the same register → segu_release passed hat_unload flags=0, keepcnt never
  released; `segu_lockfix` wrapper, commit 240204d).
...
for real-HW testing. Overlay = `src/quiet040.s` (sched loop / schedpaging skip / idle /
...
- **date/hardbus infinite-loop hang** (commit 4dc985c): a `jsr abs.L` whose extension word
...
- **"Killed" / rtld self-SIGKILL** (commit bc68e1b): anon_zero HALF-zeroed every ZFOD page
...
- **segu u-area /proc window** (commit 8dfd3b5): 3 VOP page lens 0x800→0x1000 + prumap040.s
...
- **sptmap arena free-side** (commit 28ed2c1): the kvseg arena was 4KB on the ALLOC side only;
...
are leftover debug probes in src/sigkill_dbg.s (clock_sampler, cap 1024) and mainmarks.s
...
bc68e1b).  The earlier execstk_addr / setregs-copyin-EFAULT theory was a mis-diagnosis; the real
...
The hat_free040 garbage-slot skip (commit c4265bb) WORKS: boot shows **0 kernel panics**.  hat_free
...
The gen_strategy d3 #4->#8 fix (commit 74d2ba9) is BOOT-VERIFIED on the instrumented fs-uae:
...
**ptest040 (804d423) is now PAYING OFF** (init's write-protect fault is correctly classified F_PROT,
...
= click 0x1E, loads from 0xE800 = click 0x1D); fix in `src/patch_modelb_pager.py`
...
WRONG-MODEL and were **REVERTED (commit 063d90d)**.  **CORRECTED ROOT (SMOKING GUN): the interp's
...
## >>> ★★ EXEC-HEADER FRONTIER SOLVED (2026-06-24, commits e87c255/c295fb0, boot-verified) <<<
...
The init-bootstrap blocker is FIXED.  `src/wb040.s` (commit 54aece8) ports the 68040
...
**TODO:** (1) DONE (commit 470fdad) -- wb040 + getfault040/userspace040/vtop040 moved to the BASE
build; (2) DONE (commit ff6dee8) -- krnxmemflt wrapped; (3) the exec frontier = the 4KB page-cache
...
+ `relink-030-dbg.sh` (030 golden baseline); commits 1408891..efd570d on `040-switch-trace`.
...
`1408891` (init/copyout fixes), `e5a1f73` (SCSI localization).
...
1. **DTT1 user-leak** — `src/pstart040.s` DTT1 `0x807fc060 → 0x807fa060` (S=10 both →
...
2. **userspace() 040 port** — `src/userspace040.s` (--weaken userspace @0x5b5f0). Stock
...
**SCSI root-mount FIXED (vtop040, commit 645dd01).** The r=6 was NOT cache — it was the disk
...
the a2091 24-bit+chip-bounce path.** FIX: `src/vtop040.s` (--weaken vtop + --add-symbol
...
@0x599d8 → 0x59c18). git checkpoints: `645dd01` (root+copyout work), `1408891` (pre-SCSI).
...
- **040 context switch** (resume040 in `src/mainmarks.s`, --weaken resume/sched/idle):
...
- **get_fault override** (`src/getfault040.s`, --weaken): decodes the 040 format-7
...
- **hat_pteload lazy pointer-table alloc** (`src/hat040.s`): a fresh user as has an EMPTY
...
(src/patch_setbackdq_hook.py + mainmarks.s setbackdq_hook @0xd8128) prints
...
exception frames.)  Commits: bb1b087 (hat_alloc+swtch), e5eb4c9/26f4b61/db41f93 (diag).
...
AUTO-DETECTOR `src/detect_pagesize.py` (scans objdump for the 2KB idioms, attributes
...
Detector: `python3 src/detect_pagesize.py` (719-site whole-kernel report; the full
...
- **REAL-HARDWARE line: STILL PAUSED** (resume later).  On real 040 the VM port + 040
  ...
  **`docs/archive/RESUME-HERE-040-HARDWARE.md`**.  (Emulators do NOT reproduce it -- they pass p0init.)
...
descriptors) -> garbage SDE -> DT==0 -> panic.  **PORTED**: src/hat040.s now has a
...
`sh relink-040-dbg.sh` layers an instrumented **ddopen** (src/ddopen_dbg.s,
...
m68k-cbm-sysv4-gcc -m68040 -c src/pstart040.s -o build/pstart040.o
sh relink-pstart.sh build/pstart040.o          # -> build/unix-040  (uses --weaken-symbol)
python3 src/patch_pflusha_040.py         # 030 pflusha -> 040 (re-run after each relink)
python3 src/patch_pmmu_040.py            # ptest/ptest0 stubs (re-run after each relink)
python3 src/check_relink_relocs.py       # MUST print "0 complaints"
...
**`src/hat-040-port-worklist.md`**.

**DONE: `hat_pteload`** — `src/hat040.s` (040 port, byte-faithful, relocs
...
**FIX APPLIED (src/pstart040.s, tail before `jsr mlsetup`):** `addql &1,%d2 ;
...
`src/hat-040-port-worklist.md`):
- **hat_pteload** -- DONE (`src/hat040.s`, 040 walk).  NOT YET in the build -- add
...
- `docs/mmu-format-030-to-040.md` — the exact 030/040 page-table format (the spec).
- `src/pstart-040-design.md` — pstart040 design (done).
...
- `docs/boot-path-map.md`, `docs/68040-68060-support-analysis.md` — background.
