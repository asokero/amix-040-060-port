...
scanner in `docs/060-F0-MEASUREMENT-260805.md` and `test-tools/scan060.py`.
...
* `src/lmul060.s` — a portable 32×32→64 multiply, **already validated** against a `muls.l`
...
* `src/cputype060.s` — `cputype` (40/60, loader-poked) and `pcr_boot`.
...
* `src/wb040.s` now carries `x60_*` counters in the `wb_dfc_*` shape — the same idiom is
...
* Do not propose changes to `hat_*`, the fault resolvers, or anything on the memory path. This unit
  touches one vector.
* Deliverable: a spec of the same shape as `docs/060-COUNTERS-UNIT-SPEC-260805.md` — decisions,
  constraints, pre-registered readings — not code.

## Sources

* `docs/060-F0-MEASUREMENT-260805.md` (hardware measurement, scanner methodology, the objdump
  comma-vs-colon trap that made an earlier scan report "clean")
* `KNOWN-ISSUES.md` §ISSUE-34 (corrected 2026-08-05)
* `src/lmul060.s`, `src/cputype060.s`, `src/wb040.s`
* `docs/68060-prestudy.md` §3.4, `amix-kernel-analysis/vm-map/M68060-SUPPORT-LANDSCAPE.md`
