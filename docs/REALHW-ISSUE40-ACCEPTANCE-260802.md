...
**ISSUE-40 is closed.** The fix is two units: `src/legacysdt040.s` (the legacy-SDT lifetime
edge, which alone returned zero pages) and `src/ptdatfree040.s` + `hat_ptfree` V3 (the
...
units     src/legacysdt040.s, src/ptdatfree040.s, hat040.s (hat_free + hat_ptfree V3)
