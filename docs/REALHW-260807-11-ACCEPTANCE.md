...
Expectation registered before the run (`archive/NEXT-SESSION-PROMPT-260809.md` item 2): 7/7 `OK`, `0 ulp`,
...
* **wolf3d's remaining death is F2's, and it is named exactly.** Console: `u_trap WARNING: SIGKILL
  ...
  The cause is in our own source, `src/isp61_060.s:83`:
...
* **New, and it belongs to F2 rather than F3:** the vector-61 handler declines
  `muls.l (d16,%a6),%d1:%d2`. `src/isp61_060.s:83` accepts only opword `0x4C3C`
  ...
  * `_060_real_fpu_disabled` remains implemented but unexercised: `f60_fpudis_n` is 0 here too. If
    ...
    not tolerated.
* Out of scope and unchanged: **ISSUE-42** and the **68040 session** (`archive/NEXT-040-SESSION-RUNLIST.md`,
