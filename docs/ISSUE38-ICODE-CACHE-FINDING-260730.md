...
diagnosis**. The fix is `src/cb_icode040.s` in the base link, and it is **hardware-verified**:
...
**The masker is a cache instruction, not timing.** `src/assegat_dbg.s`'s `copyout` wrapper
...
`src/cb_icode040.s`, linked into the **base** build (`relink-040.sh`), with a hard check that
