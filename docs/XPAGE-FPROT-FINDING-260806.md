...
`src/wb040.s`, `Lwx_call` — the far-page resolve both tiers share:
...
src/wb040.s              Lwx_call, the hardcoded F_INVAL
...
`src/wb040.s` now contains the classification, the verification, and five counters
...
`src/wb040.s` keeps the classification, the verification and the five counters. Verified:
