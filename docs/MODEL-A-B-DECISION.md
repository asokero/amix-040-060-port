...
literature. Primary source: `src/hat-040-port-worklist.md`, lines ~296–460.
...
```text
src/patch_modelb.py          Tier-0 core conversion
src/patch_modelb_pager.py    Tier-2 pager / dir-read chain
include-modelb/ + mk_modelb_sysroot.sh   4 KiB headers for anything compiled INTO the kernel
src/check_page_geometry.sh   object-level assertion: no shift-by-11 survives
src/va2000_modelb.py         same fix for the VA2000 driver source
src/z3660_modelb.py          same fix for the two Z3660 driver sources
```
...
* `src/hat-040-port-worklist.md` — the original A-vs-B analysis, the wall, and the scope count
