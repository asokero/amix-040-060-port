...
(`REALHW-ACCEPTANCE-260801.md`). The behaviour below is **not** a regression of that acceptance —
...
`0xd860e` is OURS** (`prototypes/hat040.s`); everything else here is a stock body.
...
3. If the answer depends on a value we can read, **name the address and the expected reading**.
   ...
   `test-tools/memwatch.c` + the `i39_*` table in `prototypes/issue39_040.s`.
