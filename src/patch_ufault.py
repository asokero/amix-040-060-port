...
# from the call at 0x5a63e, relocation at 0x5a640.  src/patch_usptrap.py has already pointed
# that relocation at unt_latch; this points it at uft_latch, which tail-jumps to unt_latch,
# which tail-jumps to cmn_err.  Both instruments run and the NOTICE prints unchanged.
#
...
#   1. no relocation at 0x5a640                  -> the base drifted; the address is wrong
#   2. relocation is not R_68K_32                -> not the call operand this was measured on
#   3. current target is not unt_latch           -> see above (already-uft_latch is a skip)
#   4. uft_latch undefined                       -> ufault_dbg.o is not in the link
#   5. uft_magic is not a .data symbol           -> the block has no file storage, so
#                                                   status-facts.sh cannot read its magic out
#                                                   of the artifact and the address printed
#                                                   for it would be a guess
# and one more, because a tail jump needs a target: unt_latch must still be defined.
#
# Idempotent.  Usage: python3 patch_ufault.py <image>
import struct
import sys

IMG = sys.argv[1] if len(sys.argv) > 1 else "build/unix-040"
RELOC = 0x0005A640
OLD, NEW = "unt_latch", "uft_latch"
BLOCK_MAGIC = "uft_magic"
R_68K_32 = 1
...
def main():
    ...
    if new_idx is None:
        raise SystemExit("ABORT: %s not defined -- is ufault_dbg.o in the link?" % NEW)
    ...
    if mi is None:
        raise SystemExit("ABORT: %s not defined -- ufault_dbg.o is not in the link" % BLOCK_MAGIC)
    ...
    for i in range(rela["size"] // rela["entsize"]):
        ...
        if cur == NEW:
            print("  [skip] fatal user-fault census already chained @0x%x" % RELOC)
            return
        if cur != OLD:
            raise SystemExit(
        ...
        print("  [ok]   user-fault census chained @0x%x  %s -> %s -> cmn_err"
              % (RELOC, NEW, OLD))
