...
# from the call at 0x5a63e, relocation at 0x5a640.  Retarget it to unt_latch, which
# latches and then tail-jumps into the real cmn_err -- the NOTICE prints unchanged.
#
# The only path that reaches this code is one already reporting a fatal user fault,
# so a healthy kernel never executes a byte of it.
#
# Usage: python3 patch_usptrap.py <image>
import struct
import sys

IMG = sys.argv[1] if len(sys.argv) > 1 else "build/unix-040"
RELOC = 0x0005A640
OLD, NEW = "cmn_err", "unt_latch"
...
def main():
    ...
    for i in range(rela["size"] // rela["entsize"]):
        ...
        if cur == NEW:
            print("  [skip] PID 1 fault NOTICE already latched @0x%x" % RELOC); return
        if cur != OLD:
