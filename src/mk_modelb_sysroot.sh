...
#
# Usage:  sh src/mk_modelb_sysroot.sh
...
AMIX_SYSROOT="$OUT" m68k-cbm-sysv4-gcc $CF \
	-c "$HERE/src/modelb_geom_probe.c" -o "$HERE/build/modelb_geom_probe.o"
...
if AMIX_SYSROOT="$REAL" m68k-cbm-sysv4-gcc $CF \
	-c "$HERE/src/modelb_geom_probe.c" -o /dev/null 2>/dev/null; then
